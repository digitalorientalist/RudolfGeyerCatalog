...
function Switch_Language(language) {
    ...
    for (key in toBeTranslated) {
        ...
        document.getElementById("flagTip").innerHTML = "<img src=\"iconflag" + nextLanguage + ".svg\" height=\"16\" width=\"32\" class=\"flags\" id=\"flagTip\" )\">";
...
function Render_Table_Entry(data, i, language) {
    ...
    htmlTableCat += "<tr><td><div class=\"entry text text-left\"><p class=\"text font-weight-bold\"><svg onclick=\"Popup_More_Info(" + i + ", " + language + ")\" xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 50 50\" width=\"15px\" height=\"15px\"><path class=\"iconModal\" d=\"M25,2C12.297,2,2,12.297,2,25s10.297,23,23,23s23-10.297,23-23S37.703,2,25,2z M25,11c1.657,0,3,1.343,3,3s-1.343,3-3,3 s-3-1.343-3-3S23.343,11,25,11z M29,38h-2h-4h-2v-2h2V23h-2v-2h2h4v2v13h2V38z\"/></svg> " +
        data[i].title +
    ...
    htmlTableCat += "</p></div></td><div class=\"entry text text-right\"><td><p class=\"text blue\">"
    if (data[i].issued) {
...
function Popup_More_Info(number, language) {
    ...
    // The div already declared in the HTML is popupInfo, the div dynamically created is popupModalInfo. All of the dynamic html is inserted in popupInfo, and then popupModalInfo is made into a modal (pop-up) wih jQuery.
    htmlPopupInfo = "<div class=\"modal fade\" id=\"popupModalInfo\" tabindex=\"-1\">" +
    ...
    if (searchData[number].hasOwnProperty('author')) {
        htmlPopupInfo += authorLan + Print_Names(searchData[number].author) + "<br>";
    }
    if (searchData[number].hasOwnProperty('editor')) {
        htmlPopupInfo += editorLan + Print_Names(searchData[number].editor) + "<br>";
    }
    ...
    if (searchData[number].hasOwnProperty('number-of-volumes')) {
        htmlPopupInfo += numberVolumesLan + searchData[number]["number-of-volumes"] + "<br>";
    }
    ...
    if (searchData[number].hasOwnProperty('abstract')) {
        htmlPopupInfo += "<hr><p class=\"text-success\"><b>" + additionalLan + "</b><br>" + searchData[number].abstract + "</p>";
    }
    if (searchData[number].hasOwnProperty('language')) {
        htmlPopupInfo += "<hr>" + callNumberLan + searchData[number].language + " (" + searchData[number].type + ")<br>";
    }
    if (searchData[number].hasOwnProperty('URL')) {
        htmlPopupInfo += '<a href="' + searchData[number].URL + '" target="_blank">' + urlLan + '</a><br>';
    }
    htmlPopupInfo += "</div>" +
    ...
    // Pretty much sole reason we included jQuery; to make this work.
